...
class TerrainWidget:
    ...
    def create(self, radius):
        ...
        loc = cmds.spaceLocator(name=self.widget_name)[0]           #a "locator"

        cmds.setAttr(loc + ".ty", lock=True, keyable=False, channelBox=False)   #no Y-axis
    ...
    def create_radius_visual(self, radius):             #a circle can be moved w/ locator
        ...
        cmds.parent(circle, self.widget_name)           #parent to locator
...
class TerrainTool:
    ...
    def get_selected_mesh(self):
        ...
        if not dag.node().hasFn(om.MFn.kMesh):
            om.MGlobal.displayError("Please select a polygon mesh (not a locator).")
            return None
    ...
    def run(self):
        ...
        self.widget.update_radius(self.radius)          #Update circle

        verts = self.get_vertices(mesh_fn)
        mesh_name = mesh_fn.name()

        # Store original per mesh
...
def create_widget():
    tool.widget.create(tool.radius)
    #
